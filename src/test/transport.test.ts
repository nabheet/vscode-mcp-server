...
/** Shape of a CallToolResult returned by handleRequest. */
...
describe("handleRequest", () => {
  ...
  it("lists all registered tools", async () => {
    ...
    const result = res.result as ListToolsResult;
  ...
  it("tools/list items have name, description, inputSchema", async () => {
    ...
    const item = (res.result as ListToolsResult).tools[0];
  ...
  it("responds to initialize with protocol version and capabilities", async () => {
    ...
    expect(res.id).toBe(1);
    expect(res.result).toBeDefined();
    const r = res.result as InitializeResult;
  ...
  it("requires object params", async () => {
    const res = await handleRequest(
      '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":"bogus"}',
      tools,
    );
    expect((res.result as ToolResult).isError).toBe(true);
    expect((res.result as ToolResult).content[0].text).toMatch(/invalid params/i);
  });
  ...
  it("requires name in params", async () => {
    ...
    expect((res.result as ToolResult).isError).toBe(true);
    expect((res.result as ToolResult).content[0].text).toMatch(/"name".*non-empty/i);
  });
  ...
  it("returns error for unknown tool name", async () => {
    ...
    expect((res.result as ToolResult).isError).toBe(true);
    expect((res.result as ToolResult).content[0].text).toMatch(/tool not found.*nope/i);
  });
  ...
  it("passes when required args are present", async () => {
    ...
    expect((res.result as ToolResult).content[0].text).toBe("Hi there");
  });
  ...
  it("rejects missing required args", async () => {
    ...
    expect((res.result as ToolResult).isError).toBe(true);
    expect((res.result as ToolResult).content[0].text).toMatch(/missing required.*name/i);
  });

  it("rejects wrong type for string arg", async () => {
    const res = await handleRequest(
      '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"hello","arguments":{"name":42}}}',
      tools,
    );
    expect((res.result as ToolResult).isError).toBe(true);
    expect((res.result as ToolResult).content[0].text).toMatch(/expected type.*string.*number/i);
  });
  ...
  it("accepts integer values for integer type", async () => {
    ...
    expect((res.result as ToolResult).isError).toBeFalsy();
  });
  ...
  it("rejects float for integer type", async () => {
    ...
    const res = await handleRequest(
      '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"add","arguments":{"a":3.5}}}',
      localTools,
    );
    expect((res.result as ToolResult).isError).toBe(true);
    expect((res.result as ToolResult).content[0].text).toMatch(/expected type.*integer.*number/i);
  });
  ...
  it("accepts number type args", async () => {
    ...
    const res = await handleRequest(
      '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"calc","arguments":{"x":3.14}}}',
      localTools,
    );
  ...
  it("wraps handler errors in result", async () => {
    ...
    expect((res.result as ToolResult).isError).toBe(true);
    expect((res.result as ToolResult).content[0].text).toMatch(/tool error.*something broke/i);
  });
  ...
  it("handles non-Error thrown values", async () => {
    ...
    const res = await handleRequest(
      '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"fail2"}}',
      localTools,
    );
    expect((res.result as ToolResult).isError).toBe(true);
    expect((res.result as ToolResult).content[0].text).toMatch(/raw string error/i);
  });
  ...
  it("handles deeply nested JSON without stack overflow", async () => {
    const deep: Record<string, unknown> = { jsonrpc: "2.0", id: 1, method: "tools/list" };
  ...
  it("handles empty arguments object", async () => {
    const handler = vi.fn().mockResolvedValue({ content: [{ type: "text", text: "ok" }] });
    const localTools = setupTools([
      makeTool("noop", { type: "object", properties: {}, required: [] }, handler),
    ]);
    const res = await handleRequest(
      JSON.stringify({
        jsonrpc: "2.0",
