...
/** Shape of the JSON-RPC response body the server returns. */
...
function _get(url: string): Promise<{ status: number; body: unknown }> {
  return new Promise((resolve, reject) => {
    http
      .get(url, (res) => {
        const chunks: Buffer[] = [];
...
describe("McpServer", () => {
  ...
  it("responds 415 for non-JSON content type", async () => {
    server = new McpServer({ port, host: "127.0.0.1" });
    await server.start();
    const res = await post(`http://127.0.0.1:${port}/mcp`, "plain text", {
  ...
  it("has no tools when none registered", async () => {
    ...
    expect((res.body as McpResponse).result.tools).toEqual([]);
  });
  ...
  it("returns registered tools", async () => {
    ...
    expect(res.status).toBe(200);
    const tools = (res.body as McpResponse).result.tools;
  ...
  it("calls a registered tool and returns result", async () => {
    ...
    const handler = vi
      .fn()
      .mockResolvedValue({ content: [{ type: "text", text: "hello from tool" }] });
    server.registerTool({
      name: "greet",
      description: "Greets",
    ...
    expect(res.status).toBe(200);
    expect((res.body as McpResponse).result.content[0].text).toBe("hello from tool");
  ...
  it("returns 400 for schema validation error", async () => {
    ...
    expect((res.body as McpResponse).result.isError).toBe(true);
  });
  ...
  it("accepts correct auth token", async () => {
    ...
    expect(res.status).toBe(200);
    expect((res.body as McpResponse).result).toBeDefined();
  });
  ...
  it("handles concurrent requests", async () => {
    ...
    for (const res of results) {
      expect(res.status).toBe(200);
      expect((res.body as McpResponse).result.content[0].text).toMatch(/count:/);
    }
  ...
  it("concurrent tools/list requests do not interfere", async () => {
    ...
    for (const res of results) {
      expect(res.status).toBe(200);
      expect((res.body as McpResponse).result.tools).toHaveLength(1);
    }
  });
  ...
  it("drains active requests before full stop", async () => {
    ...
    expect((slowResult.body as McpResponse).result.content[0].text).toBe("done");
  });
  ...
  it("uses per-tool timeoutMs override instead of the default 30s deadline", async () => {
    ...
    expect((res.body as McpResponse).error?.message).toContain("timed out after 200ms");
