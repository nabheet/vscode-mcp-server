...
async function readWorkspaceFileLaunchSection(): Promise<WorkspaceLaunchSection | undefined> {
  ...
  const wsFile = vscode.workspace.workspaceFile as unknown as
    | { uri?: vscode.Uri }
    | vscode.Uri
    | undefined;
 ...
 */
async function readFolderLaunchSection(
  folder: vscode.WorkspaceFolder,
): Promise<WorkspaceLaunchSection | undefined> {
  try {
...
export function registerDebugTools(server: ToolRegistrar): void {
    ...
    defineTool(
      ...
      "Start a debug session using a launch config name. Works with configs from .vscode/launch.json and the workspace file (*.code-workspace), including compound configurations.",
      {
        ...
        properties: {
          configName: {
            type: "string",
            description: "Launch configuration name from launch.json or the workspace file",
          },
      ...
      async (args) => {
        ...
        try {
            ...
            await withLaunchBudget(
              async (isAborted) => {
                  ...
                  try {
                    if (cfg) {
                      success = await vscode.debug.startDebugging(
                        tf,
                        cfg as vscode.DebugConfiguration,
                      );
                    } else {
                ...
                if (
                  !success &&
                  wsSection &&
                  hasLaunchEntry(wsSection, configName) &&
                  !isAborted()
                ) {
          ...
          if (!success) {
            ...
            const where =
              sources.length > 0
                ? ` (checked ${sources.join(" and ")})`
                : " (no launch sources found)";
            return {
          ...
          return {
            content: [
              {
                type: "text",
                text: `Debug start error: ${err instanceof Error ? err.message : String(err)}`,
              },
            ],
            isError: true,
          };
  ...
  const stepCommands = [
    {
      name: "step_over",
      cmd: "workbench.action.debug.stepOver",
      desc: "Step over the current line in the debugger.",
    },
    {
      name: "step_into",
      cmd: "workbench.action.debug.stepInto",
      desc: "Step into the function at the current line.",
    },
    {
      name: "step_out",
      cmd: "workbench.action.debug.stepOut",
      desc: "Step out of the current function.",
    },
    {
      name: "continue",
      cmd: "workbench.action.debug.continue",
      desc: "Continue execution in the debugger.",
    },
  ];

  for (const sc of stepCommands) {
    server.registerTool(
      defineTool(sc.name, sc.desc, { type: "object", properties: {} }, async () => {
    ...
    defineTool(
      ...
      "Add a breakpoint at a file and line. Absolute paths may point outside the workspace (the debug adapter decides whether the file is debuggable).",
      {
        ...
        properties: {
          path: {
            type: "string",
            description: "File path (absolute — any location; or relative to workspace root)",
          },
          line: { type: "integer", description: "Line number (1-indexed)" },
          condition: {
            type: "string",
            description: 'Optional breakpoint condition expression (e.g. "x > 5")',
          },
          hitCondition: {
            type: "string",
            description: 'Optional hit count condition (e.g. "5" for every 5th hit)',
          },
          workspaceFolder: {
            type: "string",
            description:
      ...
      async (args) => {
        ...
        try {
          ...
          return {
            content: [{ type: "text", text: `Breakpoint added at ${uri.fsPath}:${args.line}` }],
            isError: false,
          };
        } catch (err) {
          return {
            content: [
              {
                type: "text",
                text: `Failed to add breakpoint: ${err instanceof Error ? err.message : String(err)}`,
              },
            ],
            isError: true,
          };
      ...
      async (args) => {
        ...
        try {
          ...
          if (toRemove.length === 0) {
            return {
              content: [{ type: "text", text: `No breakpoint at ${uri.fsPath}:${args.line}` }],
              isError: true,
            };
          }
          vscode.debug.removeBreakpoints(toRemove);
          return {
            content: [{ type: "text", text: `Breakpoint removed at ${uri.fsPath}:${args.line}` }],
            isError: false,
          };
        } catch (err) {
          return {
  ...
  server.registerTool(
    defineTool(
      ...
      async () => {
        ...
        if (bps.length === 0)
          return { content: [{ type: "text", text: "No breakpoints" }], isError: false };
