    ...
    defineTool(
      ...
      "Search file contents across the workspace (grep). Returns each match with file path, line number, column, and the matching line text. Supports optional regex, case sensitivity, include/exclude globs, and context lines.",
      {
        ...
        properties: {
          ...
          exclude: {
            type: "string",
            description: 'Glob of files to skip (default: "**/node_modules/**")',
          },
          ...
          workspaceFolder: {
            type: "string",
            description:
              "Optional workspace folder name (for multi-root workspaces). Searches this folder only.",
          },
        },
        required: ["query"],
      },
        ...
        try {
          ...
          return {
            content: [
              {
                type: "text",
                text: `Invalid regular expression: ${err instanceof Error ? err.message : String(err)}`,
              },
            ],
            isError: true,
          };
        ...
        try {
          ...
          return {
            content: [
              {
                type: "text",
                text: `Failed to scan files: ${err instanceof Error ? err.message : String(err)}`,
              },
            ],
            isError: true,
          };
