...
function platformLogsRoot(): string | undefined {
  ...
  switch (process.platform) {
    ...
    case "win32":
      candidates.push(
        path.join(process.env.APPDATA || path.join(home, "AppData", "Roaming"), "Code", "logs"),
      );
      break;
    default:
  ...
  candidates.push(
    candidates[0].replace(`${path.sep}Code${path.sep}`, `${path.sep}Code - Insiders${path.sep}`),
  );
        ...
        if (!logsRoot) {
          ...
          if (newest) {
            ...
            for (const f of files) {
              ...
              out.push(
                `  ${f}  ${formatBytes(st.size)}  ${st.mtime.toISOString().replace("T", " ").slice(0, 19)}`,
              );
            }
          }
        }
  ...
  server.registerTool(
    defineTool(
        ...
        "Symlinks and paths escaping the logs root are rejected.",
      {
        ...
        properties: {
          file: {
            type: "string",
            description:
              'Log file path relative to the VS Code logs root (e.g. "2026-08-22-14:03/main.log")',
          },
          lines: {
            type: "integer",
            description: "Max lines to return, tail (default 200, max 2000)",
          },
      ...
      async (args) => {
        ...
        if (!logsRoot) {
          return {
            content: [{ type: "text", text: "VS Code logs root not found on this platform" }],
            isError: true,
          };
        }
        ...
        if (candidate !== logsRoot && !candidate.startsWith(resolved)) {
          return {
            content: [{ type: "text", text: "Rejected: path escapes the VS Code logs root" }],
            isError: true,
          };
        }
        if (!fs.existsSync(candidate) || !fs.statSync(candidate).isFile()) {
          return {
            content: [{ type: "text", text: `Log file not found: ${fileArg}` }],
            isError: true,
          };
        }
        try {
          ...
          if (real !== realRoot && !real.startsWith(realRoot + path.sep)) {
            return {
              content: [{ type: "text", text: "Rejected: symlink escapes the VS Code logs root" }],
              isError: true,
            };
          }
        } catch {
          return {
            content: [{ type: "text", text: `Cannot resolve log file: ${fileArg}` }],
            isError: true,
          };
        }
        ...
        try {
          ...
          return {
            content: [
              {
                type: "text",
                text: `--- ${candidate.replace(logsRoot + path.sep, "")} — last ${maxLines} lines${label} ---\n${text}`,
              },
            ],
            isError: false,
          };
